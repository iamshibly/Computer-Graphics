...
bool movingBack = false;      // Controls return movement
...
void update(int value) {
    if (movingToCenter) {
        ...
        if (x_left > 0) x_left -= speed;
        if (x_right < 500) x_right += speed;

        // Stop moving when both balls reach their original positions
        if (x_left <= 0 && x_right >= 500) {
            movingBack = false; // Stop movement
        }
    }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Left-Right Collision Stop and Return");
    myInit();
